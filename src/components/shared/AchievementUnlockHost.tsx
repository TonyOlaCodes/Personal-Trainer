        ...
        <ModalOverlay open onClose={() => void dismiss()}>
            <div
                className={cn(
                    ...
                    tokens.border,
                    tokens.glowStrong,
                    isLegendary && `${LEGENDARY_SHIMMER_CLASS} rarity-shimmer-active`
                )}
            ...
            >
                <button
                    ...
                    className="absolute top-4 right-4 z-10 btn-icon"
                    aria-label="Close"
                >
                    <X className="w-5 h-5" />
                </button>

                <div className={cn("absolute inset-0 pointer-events-none", tokens.cardBg)} />

                <p className={cn(
                    "relative text-[10px] font-black uppercase tracking-[0.2em]",
                    tokens.text
                )}>
                    {headlineFor(current)}
                </p>
                ...
                <div className={cn(
                    "relative mx-auto mt-5 w-20 h-20 rounded-3xl border-2 flex items-center justify-center",
                    tokens.iconWrap,
                    tokens.glow,
                    isLegendary && "scale-110"
                )}>
                    <Icon className={cn("w-10 h-10", tokens.icon)} />
                </div>

                <h3 className="relative mt-5 text-2xl font-black text-fg tracking-tight">{current.name}</h3>
                <p className={cn("relative mt-2 text-sm font-black uppercase tracking-[0.2em]", tokens.text)}>
                    {tokens.label}
                    {current.prestigeValue != null ? ` · ${current.prestigeValue}` : ""}
                </p>
                <p className="relative mt-3 text-sm text-fg-muted leading-relaxed">
                ...
                {queue.length > 1 && (
                    <p className="relative mt-4 text-[10px] font-bold text-fg-subtle uppercase tracking-widest">
